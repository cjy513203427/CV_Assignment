    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if os.path.splitext(file)[1] == ext:
                ...
                # aa = np.reshape(image_arr,(1,image_arr.shape[0],image_arr.shape[1]))
                image_list.append(image_arr)

    com_list = []
    for i in range(0,len(image_list)):
        if i%2==0:
            com_list.append(image_list[i])
        ...
        for i in range(rows):
            for j in range(cols):
                if i<cols and j<cols:
                    # M=H*W
                    imgs[g, i, j] = imgs[g, i, j]*imgs[g,j,i]


    imgs = np.reshape(imgs,(groups,-1))
    return imgs
