...
def compute_hessian(img, gauss, fx, fy):
    ...
    # use the gaussian filter and derivative filters(fx and fyy) to compute the derivatives. 
    convolved_img = convolve(img, gauss,mode='mirror',cval=0)
    I_x = convolve(convolved_img, fx,mode='mirror',cval=0)
    I_y = convolve(convolved_img, fy,mode='mirror',cval=0)
    I_xx = convolve(I_x, fx,mode='mirror',cval=0)
    I_xy = convolve(I_x, fy,mode='mirror',cval=0)
    I_yy = convolve(I_y, fy,mode='mirror',cval=0)
...
def compute_criterion(I_xx, I_yy, I_xy, sigma):
    ...
    # first_row = np.hstack((I_xx,I_xy))
    # second_row = np.hstack((I_xy,I_yy))
    # hessian_m = np.vstack((first_row,second_row))
    # scaled_det = hessian_m * pow(sigma,4)
    # # narrow one times
    # return scaled_det[::2,::2]
...
def nonmaxsuppression(criterion, threshold):
    ...
    ## TODO how can i performe nms on detection of interest points
    row_order = np.argsort(criterion[-1,:])
    rows = maximum_filter(row_order, size=5)

    col_order = np.argsort(criterion[:,-1])
    cols = maximum_filter(col_order, size=5)

    return np.shape(rows)[0], np.shape(cols)[0]