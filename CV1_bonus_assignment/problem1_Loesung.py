...
class Network:
    ...
    def init_wb(self, network_structure):
        ...
        for i in range(1, self.num_layers + 1):
            self.w[i] = np.random.randn(network_structure[i], network_structure[i-1]) / np.sqrt(network_structure[i-1])
    ...
    def layer_backward(self, dx_out, func, i):
        ...
        dz_in = self.activation_func_backward(func, dx_out, z)

        self.dw[i] = np.dot(dz_in, x_in.T)
        self.db[i] = np.sum(dz_in, axis=1, keepdims=True)
        dx_in = np.dot(w.T, dz_in)
        return dx_in
